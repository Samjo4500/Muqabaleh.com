...
type Scene = 'setup' | 'interview' | 'passport' | 'jeannie' | 'apply';
type Beat = {
  scene: Scene;
...
function MiniScene({ kind, active }: { kind: Scene; active: boolean }) {
  ...
  if (kind === 'jeannie') {
    return (
      <div className="mq-seeker-scene flex-row items-center gap-3 px-4">
        <motion.span
          className="relative h-12 w-12 shrink-0 overflow-hidden rounded-full border border-teal-300/40"
          animate={active ? { scale: [1, 1.06, 1] } : { scale: 1 }}
          transition={{ duration: 1.8, repeat: Infinity }}
        >
          <span className="absolute inset-0 bg-gradient-to-br from-teal-300/30 to-cyan-400/10" />
  ...
  return (
    <div className="mq-seeker-scene items-center justify-center gap-2">
      ...
      <motion.div className="h-1.5 w-24 overflow-hidden rounded-full bg-white/10" aria-hidden>
        <motion.span
...
export function CrystalHowItWorks() {
  ...
  const allBeats: Beat[] = C.how.steps.map((b) => ({
    scene: b.scene,
          ...
          <div className="relative grid gap-0 lg:grid-cols-[1.05fr_0.95fr]">
            <div className="relative min-h-[340px] border-b border-white/10 p-6 md:min-h-[420px] md:p-10 lg:border-b-0 lg:border-e">
              ...
              <motion.div
                ...
                animate={
                  active >= 2 ? { opacity: 1, y: 0, scale: 1 } : { opacity: 0, y: 16, scale: 0.92 }
                }
                transition={{ duration: 0.55, ease: easeCrystal }}
              >
                <MuqabalehScoreBadge
                  score={86}
                  status={active >= 4 ? 'hired' : 'scored'}
                  locale={locale}
                  size="md"
                />
              </motion.div>
            ...
            <div className="relative p-5 md:p-8">
              <ol className="relative space-y-1">
                <div
                  className="pointer-events-none absolute start-[22px] top-3 bottom-3 w-px bg-white/10"
                  aria-hidden
                />
                <motion.div
