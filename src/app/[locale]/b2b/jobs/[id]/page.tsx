...
function statusStyle(status: string) {
  switch (status) {
    ...
    case 'invited': return 'border-gold/30 bg-gold/10 text-gold';
            ...
            {CANDIDATES.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-3 text-center font-bold text-gold">{t(`${c.prefix}Overall` as 'c1Overall')}</td>
                  <td className="px-3 py-3 text-center">
                    {c.hasScores ? (
            ...
            >
              ...
              {c.hasScores && (
                <div className="mt-3 flex items-center gap-4 text-sm">
                  <span className="text-[var(--text-muted)]">{t('colOverall')}: <span className="font-bold text-gold">{t(`${c.prefix}Overall` as 'c1Overall')}</span></span>
                  <Badge variant="outline" className={
                    t(`${c.prefix}Rec` as 'c1Rec') === t('recommendYes')
                      ? 'border-emerald/30 bg-emerald/10 text-emerald'
                      : 'border-red-500/30 bg-red-500/10 text-red-500'
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto border-white/[0.08] bg-[var(--bg-panel)] sm:max-w-2xl">
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col items-center gap-3">
              <div className="flex h-28 w-28 items-center justify-center rounded-full border-2 border-gold bg-gold/[0.06]">
                <div className="text-center">
                  <span className="text-4xl font-bold text-gold">89</span>
            ...
            {/* Company Criteria */}
            <GlowCard className="space-y-3 p-5">
              <h4 className="text-sm font-bold text-gold">{t('companyCriteria')}</h4>
