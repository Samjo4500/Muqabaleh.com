              ...
              <div className="flex w-full items-center gap-2">
                <div
                  className={`h-2 flex-1 rounded-full transition-colors ${
                    i + 1 <= step ? 'bg-teal-500' : 'bg-white/10'
                  }`}
                />
              </div>
              <span
                className={`text-xs font-medium transition-colors ${
                  i + 1 === step ? 'text-teal-300' : 'text-[var(--text-faint)]'
                }`}
              >
                {t(s)}
              </span>
                ...
                {PLANS.map((plan) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div
                          className={`flex h-5 w-5 items-center justify-center rounded-full border-2 transition-colors ${
                            form.plan === plan.key ? 'border-teal-300/50' : 'border-white/20'
                          }`}
                        >
                          {form.plan === plan.key && (
                      ...
                      <span className="text-sm font-bold text-teal-300">{t(plan.price)}</span>
                    </div>
              ...
              <div className="space-y-3">
                ...
                {form.emails.length < 10 && (
                  <button
                    type="button"
                    onClick={addEmail}
                    className="flex items-center gap-2 text-sm text-teal-300 transition-colors hover:text-cyan-300"
                  >
